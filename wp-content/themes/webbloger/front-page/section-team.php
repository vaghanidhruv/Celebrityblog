<div class="front_page_section front_page_section_team<?php
	$webbloger_scheme = webbloger_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $webbloger_scheme ) && ! webbloger_is_inherit( $webbloger_scheme ) ) {
		echo ' scheme_' . esc_attr( $webbloger_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( webbloger_get_theme_option( 'front_page_team_paddings' ) );
	if ( webbloger_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$webbloger_css      = '';
		$webbloger_bg_image = webbloger_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$webbloger_anchor_icon = webbloger_get_theme_option( 'front_page_team_anchor_icon' );
	$webbloger_anchor_text = webbloger_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( webbloger_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$webbloger_css      = '';
			$webbloger_bg_mask  = webbloger_get_theme_option( 'front_page_team_bg_mask' );
			$webbloger_bg_color_type = webbloger_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $webbloger_bg_color_type ) {
				$webbloger_bg_color = webbloger_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$webbloger_caption = webbloger_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $webbloger_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $webbloger_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $webbloger_caption, 'webbloger_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$webbloger_description = webbloger_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $webbloger_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $webbloger_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $webbloger_description ), 'webbloger_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! webbloger_exists_trx_addons() ) {
						...
						webbloger_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
