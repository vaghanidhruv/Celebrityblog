...
jQuery( document ).ready(
	function() {
		...
		webbloger_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="webbloger_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.webbloger_post_editor' );
						if ( link.length > 0 ) {
							webbloger_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.webbloger_post_editor', function(e) {
						webbloger_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('webbloger_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function webbloger_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
