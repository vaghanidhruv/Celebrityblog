...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			webbloger_create_observer( 'webbloger-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				webbloger_gutenberg_editor_init();
			} );
		} else {
		 	webbloger_gutenberg_editor_init();
		}
	...
	function webbloger_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor_type == 'site' ) {
					...
					if ( editor.$frame.length && editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'webbloger_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'webbloger_inited' ) ) {
	...
	// Init on page load
	function webbloger_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = webbloger_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + WEBBLOGER_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + WEBBLOGER_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in WEBBLOGER_STORAGE['override_classes'] ) {
				$obj.addClass( WEBBLOGER_STORAGE['override_classes'][i].replace( '%s', WEBBLOGER_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( webbloger_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && WEBBLOGER_STORAGE['sidebar_position'] != 'hide' ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'webbloger_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			webbloger_remove_observer( 'webbloger-check-editor-styles-wrapper' );
			webbloger_create_observer( 'webbloger-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'webbloger_inited' );
	}
} );
