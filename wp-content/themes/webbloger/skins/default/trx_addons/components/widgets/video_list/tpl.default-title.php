	...
	if ( ! empty( $args['meta'] ) ) {
		...
		if ( is_array( $args['meta'] ) ) {
			...
			if ( ! empty( $meta_array['author'] ) ) {
				$titles_content .= '<div class="author post_meta_item"><span>'. esc_html__('By ', 'webbloger') .'</span><span class="author_name">' . esc_html($meta_array['author']) . '</span></div>';
			}
			...
			$titles_content .= '<div class="post_meta_item">' . trim( $args['meta'] ) . '</div>';
		}
		$titles_content .= '</div>';
	} 
}
