...
if ( ! function_exists( 'webbloger_storage_get' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_get( $var_name, $default = '' ) {
		global $WEBBLOGER_STORAGE;
		return isset( $WEBBLOGER_STORAGE[ $var_name ] ) ? $WEBBLOGER_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'webbloger_storage_set' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_set( $var_name, $value ) {
		global $WEBBLOGER_STORAGE;
		$WEBBLOGER_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'webbloger_storage_empty' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $WEBBLOGER_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $WEBBLOGER_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $WEBBLOGER_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $WEBBLOGER_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'webbloger_storage_isset' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $WEBBLOGER_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $WEBBLOGER_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $WEBBLOGER_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $WEBBLOGER_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'webbloger_storage_unset' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $WEBBLOGER_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			unset( $WEBBLOGER_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			unset( $WEBBLOGER_STORAGE[ $var_name ][ $key ] );
		} else {
			unset( $WEBBLOGER_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'webbloger_storage_inc' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_inc( $var_name, $value = 1 ) {
		global $WEBBLOGER_STORAGE;
		if ( empty( $WEBBLOGER_STORAGE[ $var_name ] ) ) {
			$WEBBLOGER_STORAGE[ $var_name ] = 0;
		}
		$WEBBLOGER_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'webbloger_storage_concat' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_concat( $var_name, $value ) {
		global $WEBBLOGER_STORAGE;
		if ( empty( $WEBBLOGER_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'webbloger_storage_get_array' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $WEBBLOGER_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $WEBBLOGER_STORAGE[ $var_name ][ $key ] ) ? $WEBBLOGER_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $WEBBLOGER_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $WEBBLOGER_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'webbloger_storage_set_array' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_set_array( $var_name, $key, $value ) {
		global $WEBBLOGER_STORAGE;
		if ( ! isset( $WEBBLOGER_STORAGE[ $var_name ] ) ) {
			$WEBBLOGER_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$WEBBLOGER_STORAGE[ $var_name ][] = $value;
		} else {
...
if ( ! function_exists( 'webbloger_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function webbloger_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $WEBBLOGER_STORAGE;
		if ( ! isset( $WEBBLOGER_STORAGE[ $var_name ] ) ) {
			$WEBBLOGER_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $WEBBLOGER_STORAGE[ $var_name ][ $key ] ) ) {
			$WEBBLOGER_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( '' === $key2 ) {
			$WEBBLOGER_STORAGE[ $var_name ][ $key ][] = $value;
		} else {
			$WEBBLOGER_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
		}
	}
}

if ( ! function_exists( 'webbloger_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_merge_array( $var_name, $key, $value ) {
		global $WEBBLOGER_STORAGE;
		if ( ! isset( $WEBBLOGER_STORAGE[ $var_name ] ) ) {
			$WEBBLOGER_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$WEBBLOGER_STORAGE[ $var_name ] = array_merge( $WEBBLOGER_STORAGE[ $var_name ], $value );
		} else {
			$WEBBLOGER_STORAGE[ $var_name ][ $key ] = array_merge( $WEBBLOGER_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'webbloger_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $WEBBLOGER_STORAGE;
		if ( ! isset( $WEBBLOGER_STORAGE[ $var_name ] ) ) {
			$WEBBLOGER_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			webbloger_array_insert_after( $WEBBLOGER_STORAGE[ $var_name ], $after, $key );
		} else {
			webbloger_array_insert_after( $WEBBLOGER_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'webbloger_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function webbloger_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $WEBBLOGER_STORAGE;
		if ( ! isset( $WEBBLOGER_STORAGE[ $var_name ] ) ) {
			$WEBBLOGER_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			webbloger_array_insert_before( $WEBBLOGER_STORAGE[ $var_name ], $before, $key );
		} else {
			webbloger_array_insert_before( $WEBBLOGER_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'webbloger_storage_push_array' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_push_array( $var_name, $key, $value ) {
		global $WEBBLOGER_STORAGE;
		if ( ! isset( $WEBBLOGER_STORAGE[ $var_name ] ) ) {
			$WEBBLOGER_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $WEBBLOGER_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $WEBBLOGER_STORAGE[ $var_name ][ $key ] ) ) {
				$WEBBLOGER_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $WEBBLOGER_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'webbloger_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $WEBBLOGER_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $WEBBLOGER_STORAGE[ $var_name ] ) && is_array( $WEBBLOGER_STORAGE[ $var_name ] ) && count( $WEBBLOGER_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $WEBBLOGER_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $WEBBLOGER_STORAGE[ $var_name ][ $key ] ) && is_array( $WEBBLOGER_STORAGE[ $var_name ][ $key ] ) && count( $WEBBLOGER_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $WEBBLOGER_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'webbloger_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function webbloger_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $WEBBLOGER_STORAGE;
		if ( ! isset( $WEBBLOGER_STORAGE[ $var_name ] ) ) {
			$WEBBLOGER_STORAGE[ $var_name ] = array();
		}
		if ( empty( $WEBBLOGER_STORAGE[ $var_name ][ $key ] ) ) {
			$WEBBLOGER_STORAGE[ $var_name ][ $key ] = 0;
		}
		$WEBBLOGER_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'webbloger_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function webbloger_storage_concat_array( $var_name, $key, $value ) {
		global $WEBBLOGER_STORAGE;
		if ( ! isset( $WEBBLOGER_STORAGE[ $var_name ] ) ) {
			$WEBBLOGER_STORAGE[ $var_name ] = array();
		}
		if ( empty( $WEBBLOGER_STORAGE[ $var_name ][ $key ] ) ) {
			$WEBBLOGER_STORAGE[ $var_name ][ $key ] = '';
		}
		$WEBBLOGER_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'webbloger_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $WEBBLOGER_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $WEBBLOGER_STORAGE[ $var_name ] ) ? $WEBBLOGER_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $WEBBLOGER_STORAGE[ $var_name ] ) ? $WEBBLOGER_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'webbloger_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function webbloger_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $WEBBLOGER_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $WEBBLOGER_STORAGE[ $var_name ]->$prop ) ? $WEBBLOGER_STORAGE[ $var_name ]->$prop : $default;
	}
}
