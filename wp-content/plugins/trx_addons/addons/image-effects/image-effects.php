				...
				foreach ( $wrappers as $wrapper ) {
					if ( strpos( $html, $wrapper ) !== false ) {
						...
						do {
							...
							if ( $pos !== false ) {
								$pos_img_start = strpos( $html, '<img', $pos + 20 );
								if ( $pos_img_start !== false ) {
									$pos_img_end = strpos( $html, '>', $pos_img_start + 4 );
									foreach ( $atts as $att => $new_att ) {
										$pos_att = strpos( $html, $att, $pos_img_start + 4 );
										if ( $pos_att !== false && $pos_att < $pos_img_end ) {
											$html = substr_replace( $html, $new_att, $pos_att, strlen( $att ) );
										}
									}
								}
							}
						} while ( $pos !== false );
						// Replace atts in the original image tag with the image effect and add an attribute fetchpriority="high" to the image tag if it's not present
						$html = preg_replace_callback( '/<img[^>]*trx_addons_image_effect_original_image[^>]*>/i', function( $matches ) use ( $atts ) {
							if ( ! empty( $matches[0] ) ) {
								// Replace atts
								foreach ( $atts as $att => $new_att ) {
									$matches[0] = str_replace( $att, $new_att, $matches[0] );
								}
								// Add fetchpriority="high" to the image tag if it's not present and the page is not in the Elementor's preview mode
								if ( strpos( $matches[0], 'fetchpriority=' ) === false && ! trx_addons_is_preview( 'elementor' ) ) {
