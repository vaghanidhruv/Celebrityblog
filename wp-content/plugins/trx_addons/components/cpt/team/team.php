	...
	function trx_addons_cpt_team_init() {
		...
		trx_addons_meta_box_register(TRX_ADDONS_CPT_TEAM_PT, array(
			...
			'team_form' => array(
				...
				"desc" => wp_kses_data( __("The form to contact this particular team member. If 'Inherit' is selected - the form specified in the theme options will be used. ", 'trx_addons') ),
				"std" => '',
				"options" => array(),
				"type" => "select"
			),
	...
	function trx_addons_cpt_team_options_get_list_choises($list, $name) {
		if ( is_array( $list ) && count( $list ) == 0 ) {
			if ($name == 'user') {
				...
				$edit_mode = ( strpos( $_SERVER['REQUEST_URI'], 'post.php' ) !== false && trx_addons_get_value_gp('action') == 'edit' )
							|| ( strpos( $_SERVER['REQUEST_URI'], 'post-new.php' ) !== false && trx_addons_get_value_gp('post_type') == TRX_ADDONS_CPT_TEAM_PT );
				$list = trx_addons_array_merge(
						array(
							'none' => esc_html__('None', 'trx_addons'),
							'default' => esc_html__('Default', 'trx_addons')
							),
						function_exists('trx_addons_exists_cf7')
						&& trx_addons_exists_cf7()
						&& is_admin()
				...
				if ( $edit_mode ) {
					$list = trx_addons_array_merge( array( '' => esc_html__( 'Inherit', 'trx_addons' ) ), $list );
				}
				$list = apply_filters('trx_addons_filter_page_contact_form', $list, 'team' );
			}
		}
		return $list;
	}
...
if ( !function_exists( 'trx_addons_cpt_team_contact_form' ) ) {
	...
	function trx_addons_cpt_team_contact_form( $mode ) {
		if ($mode == 'team.single') {
			$form_id = '';
			// Get an individual form for this team member
			if ( get_post_type() == TRX_ADDONS_CPT_TEAM_PT ) {
				$meta = (array)get_post_meta( get_the_ID(), 'trx_addons_options', true);
				$form_id = ! empty( $meta['team_form'] ) ? $meta['team_form'] : '';
			}
			// Get a default form for all team members
			if ( empty( $form_id ) ) {
